...
int LoRaInit(int rx, int tx, int reset, int boot){
  ...
  if (LoRaCommand("p 1"   ) < 0) return(-1); // RSSI off
...
//短距離用の設定
/*
int LoRaInitShortRange(int rx, int tx, int reset, int boot){
  rxPin = rx;
  txPin = tx;
  resetPin = reset;
  bootPin = boot;
  ...
  if (LoRaCommand("2"     ) < 0) return(-1); // processor mode
  if (LoRaCommand("x"     ) < 0) return(-1); // default
  if (LoRaCommand("b 1"   ) < 0) return(-1); // bandwidth 500kHz
  if (LoRaCommand("c 7"   ) < 0) return(-1); // spread factor SF7（短距離）
  ...
  // 短距離用(極限まで省電力最適化)
  if (LoRaCommand("m 2"   ) < 0) return(-1); // 出力弱（省電力）
  if (LoRaCommand("k 1"   ) < 0) return(-1); // CRC ON
  if (LoRaCommand("a 1"   ) < 0) return(-1); // Coding rate 4/5
  if (LoRaCommand("l 1"   ) < 0) return(-1); // 省電力モードON
...
/// 送受信チャンネルの設定については
/// http://ikkei.akiba.coocan.jp/ikkei_Electronics/LR3_ENV_sample.html